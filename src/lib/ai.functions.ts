...
export const generateEmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { callAI } = await import("./ai-client.server");
...
export const researchAssist = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const user = data.context ? `Previous context:\n${data.context}\n\nUser: ${data.query}` : data.query;
    const content = await callAI([
...
1. **Smart Email Generator** (/email) — generate professional emails with Formal, Friendly, or Persuasive tone. Also has an Email Template Library at /templates.
2. **Meeting Notes Summariser** (/meetings) — extracts summary, action items, decisions, deadlines.
